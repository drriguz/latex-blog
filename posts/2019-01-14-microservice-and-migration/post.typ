...
- 即便可以，如何能删除多余的数据？
  譬如原来有200条，现在过来150条，这150条更新了，多余的50条则没有办法删除
...
- 在导入开始的时候，生成一个唯一的version，简单一点，我们根据日期来，比如$v e r s i o n = 20190101$
- 假定有1001条数据，每200个拆分成1个message，则有6个$m e s s a g e = { 1\,2\,3\,4\,5\,6 }$
- SC中，每消费完成一个message，则记录下消费的message到一个列表中，例如$m e s s a g e s 20190101 = { 1\,2 }$（例如存储在redis中）
- SM中解析完成后，告知SC所有的message，即$m e s s a g e = { 1\,2\,3\,4\,5\,6 }$
- SC收到告知后，比对$m e s s a g e s 20190101$是否与$m e s s a g e = { 1\,2\,3\,4\,5\,6 }$匹配，如果不匹配则等待（可以利用redis的BLPOP实现，如果不匹配则不断去BLPOP）
- 所有的消息都消费完成后，更新视图，将version设置为$v e r s i o n = 20190101$。如果考虑到性能问题，可以再version字段上建索引；考虑空间问题，可以在这一步以前version的数据
