...
% Blog Metadata
% ============================================================
\blogtitle{一个数据导入的有趣问题}
\blogdate{2019-01-14}
\blogtags{架构, 云计算, ETL, 微服务}
\bloglang{zh}
\blogsummary{}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{create} \KeywordTok{table}\NormalTok{ t\_raw\_config(}
\KeywordTok{id} \DataTypeTok{int} \KeywordTok{not} \KeywordTok{null} \KeywordTok{primary} \KeywordTok{key}\NormalTok{ auto\_increment,}
\NormalTok{\textasciigrave{}version\textasciigrave{} }\DataTypeTok{char}\NormalTok{(}\DecValTok{6}\NormalTok{) }\KeywordTok{not} \KeywordTok{null}\NormalTok{,}
\NormalTok{name }\DataTypeTok{varchar}\NormalTok{(}\DecValTok{50}\NormalTok{) }\KeywordTok{not} \KeywordTok{null}\NormalTok{,}
\FunctionTok{value} \DataTypeTok{varchar}\NormalTok{(}\DecValTok{100}\NormalTok{)}
\NormalTok{);}

\KeywordTok{create} \KeywordTok{view}\NormalTok{ t\_config}
\KeywordTok{as}
\KeywordTok{select} \KeywordTok{id}\NormalTok{, name, }\FunctionTok{value} \KeywordTok{from}\NormalTok{ t\_raw\_config;}
\end{Highlighting}
\end{Shaded}
