...
- 将 $\$$ 作为字符串结尾标记加入到原字符串（记为 $S$ )末尾
- 将字符串从左到右进行轮换，对于一个长度为 $N$ 的字符串，产生 $N$
  个新的字符串，记为 $S_n$
- 将 $S\,S_1 . . . S_n$ 进行字典序排序， $\$$ 权值最小放在最前面，也即
  $S_n$ 在第一个
- 取排序后的所有字符串的最后一个字符，生成一个新的字符串（记为\${\\displaystyle S^{\"}}\$
  )，即编码完成
...
S = banana, N = 6\\\\
\\begin{cases}
S\_{0} = banana\\color{blue}{\$}\\\\
S\_{1} = anana\\color{blue}{\$}b\\\\
S\_{2} = nana\\color{blue}{\$}ba\\\\
S\_{3} = ana\\color{blue}{\$}ban\\\\
S\_{4} = na\\color{blue}{\$}bana\\\\
S\_{5} = a\\color{blue}{\$}banan\\\\
S\_{6} = \\color{blue}{\$}banana
\\end{cases}
...
\\begin{cases}
S\_{6} = \$banan\\color{red}{a}\\\\
S\_{5} = a\$bana\\color{red}{n}\\\\
S\_{3} = ana\$ba\\color{red}{n}\\\\
S\_{1} = anana\$\\color{red}{b}\\\\
S\_{0} = banana\\color{red}{\$}\\\\
S\_{4} = na\$ban\\color{red}{a}\\\\
S\_{2} = nana\$b\\color{red}{a}
\\end{cases}\\\\
S^{\"} = BWT(banana) = annb\$aa
\$\$

可以看出，转换后的字符串`annb$aa`比原来的字符串重复相连的字符的确更多了。实际上#link("http://www.bzip.org/")[bzip]就是应用了BWT结合进行压缩的：
...
\\begin{cases}
S\_{6} = \\color{green}{\$}banan\\color{red}{a}\\\\
S\_{5} = \\color{green}{a}\$bana\\color{red}{n}\\\\
S\_{3} = \\color{green}{a}na\$ba\\color{red}{n}\\\\
S\_{1} = \\color{green}{a}nana\$\\color{red}{b}\\\\
S\_{0} = \\color{green}{b}anana\\color{red}{\$}\\\\
S\_{4} = \\color{green}{n}a\$ban\\color{red}{a}\\\\
S\_{2} = \\color{green}{n}ana\$b\\color{red}{a}
\\end{cases}
...
得到这个矩阵非常简单，直接将字符串 \${\\displaystyle S^{\"}}\$
排个序就可以得到：
...
- 从左边列的 $S$ 开始，找到对应的字符作为下一个字符 $C_n$
- 根据 $C_n$ 这个字符，在左边列找到对应的字符，其对应的字符即
  $C_(n - 1)$
- 以此类推，直到结尾
...
得到这个之后，从又到左得到`a$,na,na,ba,$b, an, an`，再将其排序作为第二列,
以此类推：
...
- #link("https://zh.wikipedia.org/wiki/Burrows-Wheeler%E5%8F%98%E6%8D%A2")[维基百科-Burrows-Wheeler变换]
- #link("https://www.cs.cmu.edu/~ckingsf/bioinfo-lectures/bwt.pdf")[BWT]
