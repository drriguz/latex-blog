...
<unbounded-wildcards>
使用 ? 修饰符可以用作类型转换，List\<?\>
意味着是一个未知类型的List，可能是`List<A>` 也可能是`List<B>`
...
这里调用setData则会参数类型不能匹配。为了解决这个问题，Java编译器会生成一个Bridge
method:
...
<list-vs-listobject>
#quote(block: true)[
It's important to note that List and List\<?\> are not the same. You can
insert an Object, or any subtype of \>Object, into a List. But you can
...
Object-\>Parent-\>T-\>Child 我们有这样几种写法：

- `List<?>`
  代表一种未知类型的List，可能是`List<Object>`，也可能是`List<Child>`，都可以
- `List<? extends T>`
  代表T或者T的子类的List，可以是`List<T>`，也可以是`List<Child>`
- `List<? super T>`
  代表T或者T的父类的List，可以是`List<T>，List<Parent>，List<Object>`
...
因为在desc.set()方法中，需要的是一个能够转化为T的对象的，src中\<?
extends T\> 保证了src中的元素一定是一个T。

See also:

- #link("https://docs.oracle.com/javase/tutorial/java/generics/index.html")[Lesson: Generics (Updated)]
