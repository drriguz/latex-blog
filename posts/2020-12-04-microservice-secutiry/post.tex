...
% Blog Metadata
% ============================================================
\blogtitle{Microservice中的安全策略}
\blogdate{2020-12-04}
\blogtags{架构, 云计算, 微服务, Security}
\bloglang{zh}
\blogsummary{}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Problem}]{images/microservice_security_problem.png}}
\caption{Problem}
\end{figure}
...
\section{BFF和API Gateway}\label{bffux548capi-gateway}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={API Gateway vs BFF}]{images/api_gateway_bff_confused.png}}
\caption{API Gateway vs BFF}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={API Gateway vs BFF}]{images/microservice_bff_and_api_gateway.png}}
\caption{API Gateway vs BFF}
\end{figure}

如上所示，第一种方法是对于不同的应用分别定制不同的BFF，这里实际上BFF也可以充当API Gateway的作用；而第二种对于不同的APP使用相同的API Gateway，同样也可以在其中实习定制化的逻辑，为消费者提供优化后的API。

\section{安全控制}\label{ux5b89ux5168ux63a7ux5236}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={OAuth2 in Microservices}]{images/api_gateway_oauth.png}}
\caption{OAuth2 in Microservices}
\end{figure}
...
\item
  后端service需要通过OAuth2 Server来校验access\_token，因为access\_token中并不包含任何的身份信息
\end{itemize}
...
\tightlist
\item
  \href{https://microservices.io/patterns/apigateway.html}{Pattern: API Gateway / Backends for Frontends}
\item
  \href{https://livebook.manning.com/book/microservices-security-in-action/welcome/v-8/}{Microservices Security in Action}
\end{itemize}
