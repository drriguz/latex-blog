...
#figure(image("images/PyQt_image_viewer.png", alt: "Image viewer"),
  caption: [
    Image viewer
  ]
)
...
实际上你可以直接显示一个其他控件例如QWidget,
QLabel等，它们只是控件级别，而主窗口包含菜单、状态栏等，更适合制作程序UI。然后我们主界面的控件只有一个，那就是显示图片。图片可以使用QLabel显示，虽然它更多用来显示文字：
...
以上就是一个简单的例子，完整的程序#link("/images/ImageEditor.zip")[下载]：
