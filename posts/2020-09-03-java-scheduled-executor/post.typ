...
- $T_1 = T_0 + I n i t i a l D e l a y$
- $T_2 = T_1 + P e r i o d times 1$
- $T_n = T_1 + P e r i o d times(n - 1)$

但是这里存在一个问题，就是如果当任务的执行时间大于Period的时候，会怎样执行？

实际情形就是，如果执行时间超过了period,
那么在运行结束之后，下一个任务会立即执行。而Executor的行为是根据上面的公式创建并提交任务，也就意味着，假设period是2秒钟，而第一次执行花费了5秒，那么在这段时间之内不止一个（下一次的）任务被提交，那么后面若干次都会立刻执行，如下所示：
...
- $T_1 = T_0 + I n i t i a l D e l a y$
- $T_2 = T_1 + P_0 + D e l a y$
- $T_n = T_(() n - 1)+ P_(() n - 1)+ D e l a y$
...
- #link("https://stackoverflow.com/questions/409932/java-timer-vs-executorservice")[Java Timer vs ExecutorService?]
