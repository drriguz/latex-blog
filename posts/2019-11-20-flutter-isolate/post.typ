...
#show: blog-post.with(
  title: "Flutter性能优化实践",
  date: "2019-11-20",
  tags: ("Android", "Flutter", "移动端", "闲话编程"),
  lang: "zh",
)

我的加密软件有一个登录页面，需要用户输入主密码然后验证密码之后才能进入。因为密码转换(Key
transform)过程中用到了Argon2算法，而这个算法没有原生的dart实现，所以必须要通过插件的形式来完成，为此我还专门做了一个插件#link("https://pub.dev/packages/encryptions")[encryptions]。调用插件得到秘钥这个过程大概要花个1\~4秒钟，最近在安卓真机上测试发现，这个过程中我的进度条竟然出现了卡顿，也就是说本来应该转圈圈的，结果一开始就卡住不动了，那我还需要这个加载动画干嘛呢？为此研究了一番，如何来解决这个问题。

= 背景
<背景>
先还是大致介绍一下我的场景。如图所示：

#figure(image("images/login_blocked.gif", alt: "Blocked Login UI"),
  caption: [
    Blocked Login UI
  ]
)
...
#figure(image("images/login_blocked_overlay.gif", alt: "Performance overlay"),
  caption: [
    Performance overlay
  ]
)
...
#figure(image("images/dart_devtools.png", alt: "Dart dev tools"),
  caption: [
    Dart dev tools
  ]
)
...
// Remote image removed: https://cdn.jsdelivr.net/gh/flutterchina/flutter-in-action/docs/imgs/2-12.png
// #figure(image("https://cdn.jsdelivr.net/gh/flutterchina/flutter-in-action/docs/imgs/2-12.png", alt: "Dart event model"),
//   caption: [
//     Dart event model
//   ]
// )
...
- microtask queue：
  微任务通常来自于dart内部或者手动插入`Future.microtask(…)`
...
#figure(image("images/login_unblocked.gif", alt: "Login unblocked"),
  caption: [
    Login unblocked
  ]
)

- #link("https://medium.com/dartlang/dart-asynchronous-programming-isolates-and-event-loops-bffc3e296a6a")[Dart asynchronous programming: Isolates and event loops]
- #link("https://flutter.dev/docs/cookbook/networking/background-parsing#4-move-this-work-to-a-separate-isolate")[Move this work to a separate isolate]
- #link("https://pub.dev/packages/flutter_isolate")[flutter\_isolate]
- #link("https://alvinalexander.com/dart/dart-isolates-example")[A Dart Isolates example (Actors in Dart)]
- #link("https://github.com/flutter/flutter/issues/26413")['Window\_sendPlatformMessage' (4 arguments) cannot be found]
- #link("https://stackoverflow.com/questions/54127158/flutter-window-sendplatformmessage-4-arguments-cannot-be-found")[Flutter - 'Window\_sendPlatformMessage' (4 arguments) cannot be found]
- #link("https://github.com/dart-lang/sdk/issues/35962")[Cannot send regular Dart Instance to Isolate spawned with spawnUri]
- #link("https://github.com/flutter/flutter/issues/34993")[Plugins crash with "Methods marked with \@UiThread must be executed on the main thread."]
- #link("https://book.flutterchina.club/chapter2/thread_model_and_error_report.html")[Dart单线程模型]
