...
#show: blog-post.with(
  ...
  date: "2020-03-09",
  tags: ("Docker", "OpenShift", "云计算", "容器化"),
  lang: "zh",
)

OpenShift是红帽基于Docker和Kubernetes的云开发平台即服务（PaaS）。而#link("https://www.okd.io/")[OKD(The Origin Community Distribution of Kubernetes )]即Openshift的开源版本。在本机上搭建一套完整的Openshift环境较为麻烦，有以下几种方式：
...
= 使用VirtualBox构建Openshift集群
<使用virtualbox构建openshift集群>
按照#link("https://docs.openshift.com/container-platform/3.11/install/index.html")[安装文档]应该可以在本地搭建一个集群，但是纯手动安装的话比较复杂，幸好有#link("https://github.com/eliu/openshift-vagrant")[Openshift Vagrant]这个项目可以帮助我们简单的构建出一个集群环境。
...
#figure(
  align(center)[#table(
    ...
    table.header([Node], [IP], [Role], [Instance],),
    table.hline(),
    [master.example.com], [\.100], [node, master, etcd], [4GMem, 2Core,
    40GDisk],
...
#figure(image("images/Virtualbox-cluster.png", alt: "Virtualbox"),
  caption: [
    Virtualbox
  ]
)

这里master的域名配置有个坑，那就是hostnamanger会会生成一个master.example.com的ip映射在hosts文件里面，但是这个文件开头还有127.0.0.1
指向 master.example.com，像这样：
...
这里有几点坑： \*
`containerized=false etcd_ip=192.168.11.100`这个如果不加会导致#link("https://github.com/eliu/openshift-vagrant/issues/10")["Wait for control plane pods to appear"]错误
...
#figure(image("images/Openshift-welcome.png", alt: "Openshift home"),
  caption: [
    Openshift home
  ]
)
...
Reference: \*
#link("https://blog.csdn.net/sun_qiangwei/article/details/80443943")[OpenShift 3.9 多节点集群（Ansible）安装]
