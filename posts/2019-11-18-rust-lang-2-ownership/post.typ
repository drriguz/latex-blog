...
= Ownership概念
<ownership概念>
在rust中，内存是由一个所有权管理系统进行管理的，它会使用一些由编译器在编译时生成的规则。这个内存管理系统的好处在于，不会像JVM
stop-the-world一样暂停应用程序或者使得应用程序在运行的时候变慢。那么到底怎么样去定义ownership呢？有如下的一套规则：

- rust中每一个值都有一个变量作为与其对应的owner
- 一个值在同一时间有且仅有一个owner
- 当owner离开作用域的时候，值所在的空间会被释放(调用对象的`drop`方法)

这和C++的析构函数如出一辙，称之为#emph[Resource Acquisition Is
Initialization (RAII)]
...
#figure(image("images/rust_string_mm.svg", alt: "Rust String copy"),
  caption: [
    Rust String copy
  ]
)
