...
#figure(image("images/HashMap-resize.png", alt: "HashMap resize"),
  caption: [
    HashMap resize
  ]
)
...
实际上，HashMap并不会根据你提供的`initial capacity`来初始化一个数组，而是找到一个值
$t$ 并满足 $t > = i & & t = = 2^n$（比如3对应得到4，
15对应得到16），并在第一次插入的时候进行初始化。
...
而至于为什么默认的负载因子是0.75，有人根据二项式分布算出最佳的load
factor是 $l o g(2)= 0.693$
，然后拍脑袋给出的0.75（乘以容量还可以得个整数…)。

= 树化（红黑树）
<树化红黑树>
== TREEIFY\_THRESHOLD（树化阈值）
...
理想状况下（假设基于随机hash算法节点在桶中均匀分布，且节点的个数占桶的50%，那么单个节点出现在桶中的概率为0.5），节点在hash桶中的出现的频率遵循#link("https://zh.wikipedia.org/wiki/%E6%B3%8A%E6%9D%BE%E5%88%86%E4%BD%88")[泊松分布]（
$lambda = 0.5$ )

$ P(X = k)= frac(e^(- lambda) lambda^k, k !) = frac(e^(- 0.5) 0.5^k, k !) $

意味着在load factor=0.75的情况下，hash桶中出现 $k$
个节点（冲突）的概率大致为：
...
== Treeify（树化）
<treeify树化>
通常情况下，当哈希冲突产生的时候，会被当成链表存储。这个改变是通过#link("http://openjdk.java.net/jeps/180")[JEP 180: Handle Frequent HashMap Collisions with Balanced Trees]引入的。在下面的情况下，会转换为红黑树：
...
#figure(image("images/HashMap-treeify.png", alt: "Treeify"),
  caption: [
    Treeify
  ]
)
...
= 性能分析
<性能分析>
HashMap对于`get`和`put`操作的复杂度是常数级 $O(1)$
，在最坏的情况下，因为使用了红黑树进行查找，复杂度为 $O(l o g(n))$
。

- #link("https://stackoverflow.com/questions/20448477/cant-understand-poisson-part-of-hash-tables-from-sun-documentation")[Can't understand Poisson part of Hash tables from Sun documentation]
- #link("https://stackoverflow.com/questions/10901752/what-is-the-significance-of-load-factor-in-hashmap")[What is the significance of load factor in HashMap?]
- #link("http://rabbit.eng.miami.edu/class/een318/poisson.pdf")[Testing a Hash Function using Probability]
