...
% Blog Metadata
% ============================================================
\blogtitle{C#异步编程(1)：并发的实现基础}
\blogdate{2021-09-01}
\blogtags{Async, 刨根问底, 并发编程, Concurrent}
\bloglang{zh}
\blogsummary{}
...
\section{并发的实现基础}\label{ux5e76ux53d1ux7684ux5b9eux73b0ux57faux7840}
...
\subsection{任务切换和硬件并发}\label{ux4efbux52a1ux5207ux6362ux548cux786cux4ef6ux5e76ux53d1}
...
\subsection{用户空间和内核空间}\label{ux7528ux6237ux7a7aux95f4ux548cux5185ux6838ux7a7aux95f4}
...
例如，32位的x86架构的内存寻址空间为2\^{}32 = 4G，linux将其划分为3G(用户空间)/1G(内核空间)，windows则按照2G/2G进行划分。而对于程序来说，操作系统为其分配了虚拟地址空间(virtual address space)，并映射到物理内存上，每个进程的地址空间都是独立的(0\textasciitilde2\^{}32)，从而实现了相互隔离。
...
\subsection{上下文切换}\label{ux4e0aux4e0bux6587ux5207ux6362}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/Task-switch.png}}

当发生上下文切换的时候，操作系统需要将当前任务的状态保存到PCB(process control block)中，其实就是内存中的一个区域，并加载要切换的任务的PCB。

\pandocbounded{\includegraphics[keepaspectratio]{images/Context-Switching.jpg}}
...
\subsection{编程语言对并发的支持}\label{ux7f16ux7a0bux8bedux8a00ux5bf9ux5e76ux53d1ux7684ux652fux6301}
...
\subsection{一些容易混淆和疑惑的概念}\label{ux4e00ux4e9bux5bb9ux6613ux6df7ux6dc6ux548cux7591ux60d1ux7684ux6982ux5ff5}

\subsubsection{并发和并行}\label{ux5e76ux53d1ux548cux5e76ux884c}
...
\subsubsection{同步/异步}\label{ux540cux6b65ux5f02ux6b65}

同步(synchronous)、异步(asynchronous)通常指两种不同的编程模型。举个例子，下面的一段代码:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{func }\FunctionTok{task1}\NormalTok{() \{ }\FunctionTok{print}\NormalTok{(}\StringTok{"A"}\NormalTok{) \}}
\NormalTok{func }\FunctionTok{task1}\NormalTok{() \{ }\FunctionTok{print}\NormalTok{(}\StringTok{"B"}\NormalTok{) \}}

\NormalTok{func }\FunctionTok{main}\NormalTok{() \{}
    \FunctionTok{task1}\NormalTok{()}
    \FunctionTok{task2}\NormalTok{()}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{阻塞/非阻塞}\label{ux963bux585eux975eux963bux585e}

阻塞(blocking)、非阻塞(nonblocking)通常也会跟着同步和异步一起出现。而这几个概念如果要严格区分的话，是需要在不同的上下中进行讨论的:

\paragraph{进程的阻塞}\label{ux8fdbux7a0bux7684ux963bux585e}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/Process-State.png}}
...
\paragraph{I/O的阻塞/非阻塞}\label{ioux7684ux963bux585eux975eux963bux585e}
...
\subsection{进程vs线程}\label{ux8fdbux7a0bvsux7ebfux7a0b}
...
\item
  \href{https://channel9.msdn.com/Series/Three-Essential-Tips-for-Async}{Six Essential Tips for Async}
\item
  \href{https://docs.microsoft.com/en-us/dotnet/csharp/programming-guide/concepts/async/}{Asynchronous programming with async and await}
\item
