...
还有Ini +
Json的方法，但是感觉也比较丑，于是想来想去，还不如按照自己的意愿发明一种配置文件格式好了，主要有以下的考虑：
\* 语法应该简单，不需要依赖缩进 \* 支持数组 \* 支持使用变量（类似shell）
\* 支持Unicode，中文直接写，所见即所得 \*
支持某种形式的命名空间（类似ini中的section）来对配置进行分组 \* 支持注释
\* 支持多行字符串 \* 格式好看…
