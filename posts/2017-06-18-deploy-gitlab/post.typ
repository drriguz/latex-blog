...
这个镜像大概有300多M，如果下载太慢，请参照上一篇文章说的加速的配置。下载完成后，就需要来跑起来了，参照#link("https://docs.gitlab.com/omnibus/docker/")[官方文档]。
...
启动gitlab后就可以通过 http:/\/192.168.56.101:1080
来访问了，默认的用户名是root，第一次进入会设置root密码。

#emph[备注]
经过一番折腾，如果不使用默认端口（80，443）等配置的时候有些问题没有解决，于是为了简单起见，最终使用80端口。
---

#emph[以下是可选操作，如果生成独立IP在Mac上访问虚拟机内的Docker也会存在麻烦，仅供参考]
...
就选#link("https://github.com/jpetazzo/pipework")[Pipework]吧，感觉会比较简单。
