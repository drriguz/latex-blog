...
#figure(image("images/Sqlite3-file-layout.png", alt: "Sqlite3 file layout"),
  caption: [
    Sqlite3 file layout
  ]
)
...
其中，第一个page记为page
1（而不是从0开始算）。任一个page都属于以下的某一种：

- lock-byte
  page，设计给VFS使用，本身sqlite并不需要，任何大小在1073741824(1024m=1G)以内的数据库文件都不包含lock-byte
  page。
- freelist page，数据库的空闲page（譬如删除一些数据之后，页仍然保留）
  - freelist trunk page，存储一个4位的leaf
    page数组，因最小的page可用空间为480所以至少可以存储120个entry。其中第一个id为下一个freelist
    trunk page的id，如果没有则为空
...
== 页大小（page\_size）
<页大小page_size>
页的大小必须为512\~65536间的2的整数幂。从#link("https://www.sqlite.org/releaselog/3_12_0.html")[3.12.0]开始，默认的page大小从1024调整到了4096。可以通过如下的命令来查看当前数据库的页大小：
...
#figure(
  align(center)[#table(
    ...
    table.header([range], [样例值], [description],),
    table.hline(),
    [00..15], [同→], ["SQLite format 3\\000"],
    [16..17], [0x0200], [page size（in bytes)，如果是1则表示为65536],
    [18], [0x01], [file format write
    version,1=legacy,2=WAL，用于向后兼容。若新版本可被旧版本安全读取但不可写入，则可设置为比就版本的version更高。],
    [19], [0x01], [file format read version,
    1=legacy,2=WAL，用于向后兼容，若大于2则不允许读取或者写入。],
    [20], [0x00], [每页的保留字节数（在页的末尾），通常为0，可用于存储一些额外的信息，例如让SQLite
    Encryption Extension存储checksum等。],
    [21], [0x40], [Maximum embedded payload fraction. Must be 64.
    暂时不支持更改],
    ...
    [40..43], [0x01], [schema
    cookie，每当schema变化的时候这个值就增长（prepared
    statement对应到一个schema
    version，如果schema变化也必须重新prepare)。],
    [44..47], [0x04], [schema format
    number(允许的格式为1,2,3或者4)，类似read/write
    version,用于向后兼容，以支持新的schema语法，当前最新为4（SQLite
    3.3.0 on 2006-01-10）],
    [48..51], [0x00], [默认的page缓存大小],
    [52..55], [0x03], [The page number of the largest root b-tree page
    when in auto-vacuum or incremental-vacuum modes, or zero
    otherwise.],
    [56..59], [0x01], [数据库文件编码，1=UTF-8, 2=UTF-16le, 3=UTF-16be],
    [60..63], [0x00], [user version],
    [64..67], [0x00], [True (non-zero) for incremental-vacuum mode.
...
- table b-tree:
  使用64位有符号整数作为key（也即是rowid)，数据保存在叶子节点中（内部节点中只包含key和指向子节点的指针），因此来看这是属于b+-tree结构
- index b-tree: 只存储key而没有数据

对于一个b-tree的内部节点，存储有k个key和k+1个指向子节点的指针，在sqlite中即节点的page
number。一个key以及其左边子节点的page
number组合被称之为一个单元格（cell），而最右侧的指针没有对应的key，是单独储存的。每个数据库都有两个特殊的b-tree:

- 一个table
  b-tree用来存储所有的schema，包含系统的表sqlite\_schema，这个b-tree的root
  page即第一个page，并存储了其他表和index的root page的序号
- 一个index b-tree用来存储schema中的index

除此之外，普通用户创建的表则对应到一个table
b-tree（有一个例外就是如果建表没有指定primary key,则会使用index
b-tree而不是table b-tree)。
...
- cell pointer 数组，假设page有K个cell，则存储K个2字节的，到cell
  content位置的偏移，按key升序排列
- 空闲空间
- cell content 区域
- 保留区域
...
#figure(
  align(center)[#table(
    columns: 2,
    align: (left,left,),
    table.header([range], [description],),
    table.hline(),
    [0], [page type： 0x02=index b-tree 内部页 0x05=table
    b-tree内部页，0x0a=index b-tree叶子页，0x0d=table b-tree叶子页],
...
<变长整数variable-length-integer>
为节省空间，sqlite中通过varint来存储霍夫曼编码的补码64位整数，占1-9个字节。设其从低到高分别为$A_0$,
$A_1$, \.., $A_8$, 则其解码如下：

- 若 $0 lt.eq A_0 lt.eq 240$，则$N = A_0$
- 若
  $241 lt.eq A_0 lt.eq 248$，则$N = 240 + 256 times(A_0 - 241)+ A_1$
- 若 $A_0 = 249$，则$N = 2288 + 256 times A_1 + A 2$
- 若 $A_0 = 250$，则$N = b i g - e n d i n g(A_1 - - A_3)$
- 若 $A_0 = 251$，则$N = b i g - e n d i n g(A_1 - - A_4)$
- 若 $A_0 = 252$，则$N = b i g - e n d i n g(A_1 - - A_5)$
- 若 $A_0 = 253$，则$N = b i g - e n d i n g(A_1 - - A_6)$
- 若 $A_0 = 254$，则$N = b i g - e n d i n g(A_1 - - A_7)$
- 若 $A_0 = 255$，则$N = b i g - e n d i n g(A_1 - - A_8)$
...
<overflow-page>
对于table
b-tree的叶子节点，其payload如果超过一个阈值，无法完整存储到单个page中，则会使用overflow
page链表来存储余下的部分。

设 $U$为没页的可用大小，
$P$为payload的大小，$X$为页中最大直接存储的payload大小, $M$
为最小必须存到page中的payload大小，则：

= 记录格式
<记录格式>
table b-tree中的payload（或者index
b-tree中的key）都是存储为记录格式(record
format)。每一个record包含文件头和body，依如下格式：
...
在某些情况下，值的个数可能少于column，例如通过alter
table来增加列，sqlite并未修改已有数据。这种情况下新增列的值为默认值。
...
#figure(image("images/Sqlite3-rowformat-e1.png", alt: "Example"),
  caption: [
    Example
  ]
)
...
#figure(image("images/Sqlite-split-18.png", alt: "Sqlite split"),
  caption: [
    Sqlite split
  ]
)

#link("https://www.sqlite.org/fileformat.html")[Database File Format]
#link("http://barbra-coco.dyndns.org/sqlite/fileformat.html")[]
