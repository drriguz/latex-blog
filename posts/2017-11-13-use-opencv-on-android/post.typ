...
= 导入OpenCV动态库
<导入opencv动态库>
首先当然是下载OpenCV for Android了，然后使用Android
Studio创建一个工程并勾选C++ support。
...
<导入opencv-jaba-module>
把opencv
sdk下面的java目录作为一个module导入到工程中，并设置app依赖此module，这样就可以在工程中使用opencv提供的java接口了。我们来做一个相机：
...
参考：#link("http://blog.codeonion.com/2016/04/09/show-camera-on-android-app-using-opencv-for-android/")[Use OpenCV to show camera on android App with correct orientation]
